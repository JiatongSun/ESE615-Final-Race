...
# Running this file will generate a csv file under pure_pursuit/csv/
csvfile = input_map + '_traj_race_cl.csv'
module = os.path.dirname(os.path.abspath(__file__))
csv_path = module + "/outputs/" + csvfile

# define the turns in terms of [inside_corner, outside_corner]
if input_map == 'levine_2nd':
    turns = [{'inner': np.array([0.584, 0.728]), 'outer': np.array([-0.858, -0.812])},
             {'inner': np.array([7.72, 0.603]), 'outer': np.array([9.21, -1.01])},
             {'inner': np.array([8.61, 22.2]), 'outer': np.array([10.1, 23.8])},
             {'inner': np.array([1.33, 22.5]), 'outer': np.array([-0.144, 24.1])}]
elif input_map == 'levine':
    turns = [{'inner': np.array([8.81, 0.64]), 'outer': np.array([10.48, -1.11])},
             {'inner': np.array([8.81, 7.92]), 'outer': np.array([10.48, 9.63])},
             {'inner': np.array([-12.93, 7.92]), 'outer': np.array([-14.58, 9.63])},
             {'inner': np.array([-12.93, 0.64]), 'outer': np.array([-14.58, -1.11])}]

# define velocity parameters
min_speed = 3  # the target speed at the end of braking
max_speed = 5  # the max speed on the straights
brake_dist = 4  # distance from the beginning of the turn to start braking

turn_start_dist = 0.23  # distance down the road from the inner corner to begin turn
turn_end_dist = 2.0  # distance down the road from the exit of the inner corner to end turn
throttle_aggression = 0.1  # how quickly to speed up after coasting
coast_ratio = 0.4

safe_dist = 0.5  # the distance to leave from the wall
...
class LineGenerator:
    ...
    def __init__(self, min_speed, max_speed, brake_dist, turn_start_dist, turn_end_dist, coast_ratio,
                 throttle_aggression, safe_dist, turns):
        ...
        self.num_brake_points = floor(
            brake_dist / self.dist_brake_points)  # number of waypoints to have in braking zone
    ...
    def def_turn(self, curr_turn_idx, start_vel,
                 turn_finished):  # curr_turn: index from 0 to 3 of which turn you're looking at
...
racing_line = LineGenerator(min_speed, max_speed, brake_dist, turn_start_dist, turn_end_dist, coast_ratio,
                            throttle_aggression, safe_dist, turns)
...
np.savetxt(csv_path, racing_line.waypoints, delimiter=",")
