...
class OpponentPredictor(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("wall_safe_dist")
        ...
        self.map = self.read_map(map_name, map_img_ext)
    ...
    def read_map(map_name, map_img_ext):
        ...
        return np.dstack((map_vs, map_xs, map_ys))
    ...
    def get_opponent(self):
        ...
        # Calculate all pairs distance between wall points and grid points
        d = distance.cdist(grid_point, map_point)
        dist_to_wall = np.min(d, axis=1)

        grid_safe_dist = self.get_parameter("grid_safe_dist").get_parameter_value().double_value
        wall_safe_dist = self.get_parameter("wall_safe_dist").get_parameter_value().double_value
        assert wall_safe_dist > grid_safe_dist

        # Find obstacle that is not wall
        obstacle_idx = np.where(dist_to_wall > wall_safe_dist)[0]
