...
config_file = "src/pure_pursuit/config/params.yaml"
...
CAR_MAP = input_map + '_traj_race_cl'
CSV_LOC = os.path.join('src', 'pure_pursuit', 'csv', CAR_MAP + '.csv')
...
class PurePursuit(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('lookahead_idx', 10)
        self.declare_parameter('lookahead_attenuation', 0.4)
        ...
        self.target_kappa = waypoints[:,  4]
        self.target_v = waypoints[:, 5]
        self.target_a = waypoints[:, 6]
    ...
    def get_lookahead_dist(self, curr_idx):
        ...
        yaw_before = self.target_yaw[(curr_idx - lookahead_idx) % self.num_pts]
        yaw_after = self.target_yaw[(curr_idx + lookahead_idx) % self.num_pts]
        yaw_diff = abs(yaw_after - yaw_before)
        if yaw_diff > np.pi / 2:
            yaw_diff = np.pi / 2
        L = max(0.5 * L, L * (np.pi / 2 - yaw_diff * slope) / (np.pi / 2))
